...
def run(cwd, nmgc_cmd='nmgc', verbose=True, timelimit=None):
    """Run an NMGC gas-grain chemistry simulation.

    Calls ``nmgc run`` as a subprocess in the given working directory,
    which must already contain all required input files (parameters.in,
    species files, reaction files, etc.).
    ...
    verbose : bool, optional
        ...
        output is redirected to ``<cwd>/nmgc.out``.
    ...
    if verbose:
        result = sp_run(command.split(), cwd=cwd, stderr=STDOUT, timeout=timelimit)
    else:
        with open('{}/nmgc.out'.format(cwd), 'w') as f:
            result = sp_run(command.split(), cwd=cwd, stdout=f, stderr=f, timeout=timelimit)
